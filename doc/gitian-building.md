...
*Setup instructions for a gitian build of Coin2Play using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coin2Play
...
- [Building Coin2Play](#building-coin2play)
...
Clone the git repositories for coin2play and gitian and then checkout the coin2play version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coin2play-crypto/coin2play.git
cd coin2play
...
Follow the instructions in [doc/release-process.md](release-process.md) in the coin2play repository
...
To build Coin2Play (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the coin2play repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coin2play/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/coin2play-crypto/coin2play
...
For example:
```bash
URL=https://github.com/crowning-/coin2play.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit coin2play=${COMMIT} --url coin2play=${URL} ../Coin2Play/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coin2play=${COMMIT} --url coin2play=${URL} ../Coin2Play/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coin2play=${COMMIT} --url coin2play=${URL} ../Coin2Play/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coin2play-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coin2play-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/coin2play-build.assert
```
...
[coin2play/gitian.sigs](https://github.com/coin2play-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
